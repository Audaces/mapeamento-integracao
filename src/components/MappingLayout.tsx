...
import {
  Building2, GitBranch, Monitor, BookOpen, ChevronLeft, ChevronRight, Printer, CheckCircle2, FileCheck
} from "lucide-react";
...
export default function MappingLayout() {
  ...
  const handleNext = () => {
    ...
    if (currentStep < steps.length - 1) {
      ...
      setIsFinished(true); // FINALIZA O MAPEAMENTO
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-6 animate-in zoom-in duration-300">
        ...
        <div className="no-print bg-white p-10 rounded-xl shadow-xl border border-slate-200 text-center max-w-lg w-full mb-10">
          ...
          <p className="text-gray-600 mb-8">Todos os dados foram coletados com sucesso. Agora você pode gerar o relatório oficial em PDF.</p>
          ...
          <div className="flex flex-col gap-3">
            ...
            <Button variant="ghost" onClick={() => setIsFinished(false)}>
              Revisar Dados
            </Button>
          </div>
        </div>
    ...
    <div className="flex min-h-screen bg-background">
      <aside className="no-print w-64 shrink-0 bg-white flex flex-col border-r">
        ...
        <div className="p-4 border-t space-y-3 bg-slate-50">
          ...
          <p className="text-[10px] text-center text-gray-400">Finalize o mapeamento para liberar o PDF</p>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50/30">
        <header className="no-print border-b px-6 py-2 flex items-center justify-between bg-white shadow-sm">
          <span className="text-[10px] font-bold text-gray-400">Etapa {currentStep + 1} de 4</span>
