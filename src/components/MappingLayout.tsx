...
export default function MappingLayout() {
  ...
  const handleNext = () => {
    if (!validate()) {
      ...
      if (currentStep === 2) msg = "Defina a Logística de Código, as Regras e preencha 3 campos nas categorias ativas.";
          ...
          <div className="bg-white p-8 md:p-10 rounded-xl shadow-xl border border-slate-200 text-center max-w-xl w-full">
            ...
            <div className="space-y-6 text-left mb-8">
              ...
              <div className="flex items-start gap-3 p-4 rounded-lg bg-blue-50 border border-blue-100">
                ...
                <div>
                  <p className="text-xs font-bold text-blue-800 uppercase tracking-widest mb-1">Passo 2 (Lembrete)</p>
                  <p className="text-xs text-blue-700 leading-relaxed">
                    Não esqueça de <strong>anexar o PDF</strong> gerado no Passo 1 dentro do formulário do Insights no próximo passo.
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-lg border border-slate-100 bg-slate-50/50">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Passo 3</p>
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {steps.map((step, i) => {
            ...
            return (
              <button key={step.id} 
                onClick={() => (completedSteps.has(i) || i < currentStep) && setCurrentStep(i)}
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50/30">
        <header className="no-print border-b px-4 md:px-6 py-3 flex items-center justify-between bg-white shadow-sm z-10">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={handleNext} style={{ backgroundColor: CORPORATE_BLUE }} className="text-white px-4 md:px-6 font-bold shadow-md h-8 text-xs">
