...
const exampleRows = [
  ...
  { stage: "Aprovação de Amostra", system: "Audaces Idea", area: "Qualidade / Estilo", data: "Status, comentários", obs: "Workflow" },
  { stage: "Ficha Técnica Final", system: "Isa/Idea → ERP", area: "Engenharia / PCP", data: "Ficha completa, grade", obs: "Integração" },
...
export default function Step4Workflow({ data = { benefits: "", rows: [] }, update, isPrint }: any) {
  ...
  // Inicialização das 6 linhas editáveis
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <Card className="border-primary/20 bg-primary/5 no-print">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Descreva os benefícios e mapeie o fluxo completo da ficha técnica.
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <Card className={isPrint ? "border-none shadow-none" : ""}>
        ...
        <CardContent>
          <Textarea
            ...
            placeholder="Descreva os ganhos esperados aqui..."
            className={isPrint ? "border-none p-0" : ""}
          />
        </CardContent>
      </Card>

      {/* Exemplo Ilustrativo - TEXTO CORRIGIDO PARA PRETO */}
      {!isPrint && (
        <Card className="border-accent/30 bg-accent/5">
          <CardHeader>
            <CardTitle className="text-base text-slate-900 font-bold flex items-center gap-2">📋 Exemplo Ilustrativo (apenas referência)</CardTitle>
            <CardDescription className="text-slate-700 font-medium italic">Preencha o fluxo real abaixo ↓</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {exampleRows.map((row, i) => (
                  <TableRow key={i} className="text-xs border-accent/20">
      ...
      <Card className={isPrint ? "border-none shadow-none" : ""}>
        <CardHeader>
          ...
          <CardDescription>Preencha ao menos 3 etapas (qualquer caractere é aceito)</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {rows.map((row: any, i: number) => (
                <TableRow key={i}>
                  <TableCell className="p-2"><Input value={row.stage || ""} onChange={(e) => handleRowChange(i, "stage", e.target.value)} /></TableCell>
                  <TableCell className="p-2"><Input value={row.system || ""} onChange={(e) => handleRowChange(i, "system", e.target.value)} /></TableCell>
                  <TableCell className="p-2"><Input value={row.area || ""} onChange={(e) => handleRowChange(i, "area", e.target.value)} /></TableCell>
                  <TableCell className="p-2"><Input value={row.data || ""} onChange={(e) => handleRowChange(i, "data", e.target.value)} /></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
